...
(check-equal? (evo '((λ (x) 324) "value passed into x"))
                   '(324 #hash((E1 . ((x #&"value passed into x") E0))
                               (λ0 . ((λ (x) 324) E0)))))
; function call with function-term's argument-term is a lambda-term, this lambda-term's body is an assignment-term
(check-equal? (evo '((λ (x) x) (λ (y) (set! y 200) y)))
                   '(200
                     #hash((E1 . ((y #&200) E0))
                           (E2 . ((x #&200) E1))
                           (λ0 . ((λ (x) x) E0))
                           (λ1 . ((λ (y) (set! y 200)) E0)))))
; similar to above, but has multiple body
(check-equal? (evo '((λ (x) x) (λ (y) (set! y 200) (set! z 500) y)))
                   '(200
                     #hash((E1 . ((y #&200) E0))
                           (E2 . ((x #&200) E1))
                           (E3 . ((z #&200) E2))
                           (λ0 . ((λ (x) x) E0))
                           (λ1 . ((λ (y) (set! y 200) (set! z 500)) E0)))))
; function call where the function-term's body is another function call.
(check-equal? (evo '((λ (x) (λ (y) x)) "value passed into x"))
