...
; • CSC324 — 2023W — Exercise 2 — Solution
...
; known as “point-free” programming (the parameter is considered the “point”).
; The earlier functions you defined support point-free programming: use those.

(define (tag t)
  (And list? (And (Not empty?) (compose (λ (v) (equal? t v)) first))))
...
(define (Point x y)
  (λ (msg)
    (match msg
      ...
      ('scale (λ (c) (Point (* x c) (* y c))))
      ('add (λ (p) (Point (+ x (p 'x)) (+ y (p 'y))))))))
...
; For (if conditional-expression consequent-expression alternative-expression)
...
(module+ test
  ...
  (check-equal? (tails '(if cond (if cond x y) (if cond x y))) (list 'x
                                                                   'y
                                                                   'x
                                                                   'y)))
...
(define (tails e)
  (match e
    ((list 'and expression ... consequent-expression)
     (tails consequent-expression))
    ((list 'if
           conditional-expression
           consequent-expression
           alternative-expression)
     (append (tails consequent-expression)
             (tails alternative-expression)))
