...
(define (tag t)
 (λ (lst) ( (And (Not empty?) (compose (λ (v) (equal? t v)) first)) lst)) )

;  (λ (lst) (and (pair? lst) ((compose (λ (v) (equal? t v)) first) lst) )))
  
;(lambda (lst)
 ;   (and (pair? lst)
  ;       (eq? (car lst) t))))
;  (lambda (x) ((compose (λ (v) (equal? t v)) first) x)))
;  (compose (λ (lst) (equal? t (car lst))) pair?))
...
(define (Point x y)
  (λ (msg) (match msg ['x x]
             ...
             ['scale (λ (sc) (λ (proc) (* sc ( (Point x y) proc))) ) ]
             ['add (λ (pt) (λ (proc) ((Point (+ x (pt 'x)) (+ y (pt 'y))) proc)) )  ]
             )))
;  (λ (msg)
;    msg))
...
(define (tails e)
  (match e
    [(list 'and expr consequent-expression) (tails consequent-expression)]
    [(list 'if condexpr consqexpr altexpr)  (append (tails consqexpr) (tails altexpr))]
    [_ (list e)]
    ))
